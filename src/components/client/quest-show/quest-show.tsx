...
import type { createTask, updateTask } from '@/actions'
...
export function QuestShow({ quest, tasks, createTaskAction, updateTaskAction }: QuestShowProps) {
  return (
    <PageContent className="pt-8">
      ...
      <div className="grid gap-4">
        {tasks.map(task => (
          <TaskCard key={task.id} task={task} updateTaskAction={updateTaskAction} />
        ))}
      </div>
    </PageContent>
  )
